...
def calculate_frequencies(file_contents):
    ...
    for ch in file_contents:
        if ch not in punctuations:
            strWOPunctuations += ch
    listOfWords = strWOPunctuations.split()
    listFinalWords = []
    wordFrequencyDict = {}
    for word in listOfWords:
        if word not in uninteresting_words:
            if word not in wordFrequencyDict.keys():
                wordFrequencyDict[word] = 1
            else:
                wordFrequencyDict[word] = wordFrequencyDict[word] + 1
    
    
    #wordcloud
    cloud = wordcloud.WordCloud()
    cloud.generate_from_frequencies(wordFrequencyDict)
